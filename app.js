...
window.abrirPerfil = async function(nickBuscado) {
    ...
    try {
        ...
        if (!snapshot.empty) {
            ...
            // Renderizado de Historial / Timeline de Batallas (Firestore)
            const listaHistorial = document.getElementById('lista-historial-partidos');
            if (listaHistorial) {
                const historial = data.historialPartidos || [];
                if (historial.length === 0) {
                    listaHistorial.innerHTML = `<p style="color: #888; font-size: 0.8rem; text-align: center; padding: 10px 0;">Este ninja aún no ha registrado combates oficiales.</p>`;
                } else {
                    // Ordenar por fecha más reciente
                    const historialOrdenado = [...historial].reverse();
                    listaHistorial.innerHTML = historialOrdenado.map(item => {
                        const esVic = item.resultado === 'Victoria';
                        const colorBadge = esVic ? 'var(--blue)' : 'var(--red)';
                        const iconVic = esVic ? 'fa-trophy' : 'fa-skull';

                        return `
                            <div style="background: rgba(0,0,0,0.5); border-left: 3px solid ${colorBadge}; border-radius: 4px; padding: 8px 12px; margin-bottom: 8px; display: flex; justify-content: space-between; align-items: center;">
                                <div>
                                    <div style="font-size: 0.82rem; font-weight: bold; color: white;">
                                        <i class="fas ${iconVic}" style="color: ${colorBadge}; margin-right: 5px;"></i> ${item.torneo}
                                    </div>
                                    <div style="font-size: 0.75rem; color: #888; margin-top: 2px;">
                                        vs <strong style="color: #ccc;">${item.rival}</strong> · <span>${item.fecha}</span>
                                    </div>
                                </div>
                                <div>
                                    <span style="font-size: 0.75rem; font-weight: bold; color: ${colorBadge};">${item.resultado.toUpperCase()}</span>
                                </div>
                            </div>
                        `;
                    }).join('');
...
window.setGanadorManual = async function(torneoId, partidoId, ganadorName) {
    ...
    if (partido.p2 !== "BYE") {
        ...
        // 2) ELO & Historial de Partidos: registrar resultado en el timeline del ninja
        ...
        const registrarHistorialEnFirestore = async (nick, rivalNick, gane, torneoNombre) => {
            const snap = await db.collection('ninjas').where('nick', '==', nick).get();
            if (!snap.empty) {
                const registroMatch = {
                    id: 'match_' + Date.now(),
                    torneo: torneoNombre || 'Torneo Relámpago',
                    rival: rivalNick,
                ...
                await snap.docs[0].ref.set({
                    historialPartidos: firebase.firestore.FieldValue.arrayUnion(registroMatch)
                }, { merge: true });
            }
        };

        for (const nick of nicksGanador) {
            await actualizarEloJugador(nick, eloPromPerdedor, true);
            await registrarHistorialEnFirestore(nick, perdedorName, true, torneoData.nombre);
        }
        for (const nick of nicksPerdedor) {
            await actualizarEloJugador(nick, eloPromGanador, false);
            await registrarHistorialEnFirestore(nick, ganadorName, false, torneoData.nombre);
        }
